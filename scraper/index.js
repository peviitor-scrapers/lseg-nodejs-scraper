...
async function fetchJobsPage(pageNum) {
  ...
  const url = `${JOB_BASE}/wday/cxs/lseg/Careers/jobs`;

  const payload = {
    appliedFacets: {
      locationCountry: [ROMANIA_COUNTRY_ID]
    },
    limit: PAGE_SIZE,
    offset: offset,
    searchText: ""
...
function parseApiJobs(apiData) {
  const jobs = apiData.jobPostings || [];
    ...
    jobs: jobs.map(job => {
      ...
      if (job.locationsText) {
        ...
        for (const part of parts) {
          const cityMatch = part.match(/^([A-Za-z\u00C0-\u024F\s.-]+)/);
          if (cityMatch && !part.toLowerCase().includes("location")) {
            location.push(cityMatch[1].trim());
          }
        }
        if (location.length === 0) {
          const firstPart = parts[0].trim();
          if (firstPart && !firstPart.toLowerCase().includes("location")) {
            location.push(firstPart);
          }
        }
      }
      ...
      const url = externalPath.startsWith("http")
        ? externalPath
        : `${JOB_BASE}/en-US/Careers${externalPath}`;

      const tags = [];
      for (const field of job.bulletFields || []) {
        if (field.startsWith("Division:")) {
          tags.push(field.replace("Division:", "").trim().toLowerCase());
        }
      }

      let workmode = "hybrid";
      if (job.timeType) {
        const tt = job.timeType.toLowerCase();
        if (tt.includes("remote")) workmode = "remote";
        else if (tt.includes("on-site") || tt.includes("office")) workmode = "on-site";
      }

      return {
        url,
        title: job.title,
        uid: job.bulletFields?.[0] || "",
        workmode,
