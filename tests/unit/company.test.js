...
const LSEG_ANAF_RECORD = {
  ...
  address: 'IULIU MANIU, 6F, Bucureşti Sectorul 6, Bucureşti',
  caenCode: '6220',
  inactive: false,
  vatRegistered: true,
  eFacturaRegistered: false,
  headquartersAddress: { locality: 'Bucureşti Sectorul 6' }
};
...
describe('company.js', () => {
  ...
  describe('getCompanyData (no cache)', () => {
    it('should fetch LSEG via direct CIF lookup and return company data', async () => {
      mockFetch.mockResolvedValueOnce(anafCompanyResponse(LSEG_ANAF_RECORD));

      const result = await company.getCompanyData();

      expect(result).toHaveProperty('company', 'LSEG BUSINESS SERVICES RM S.R.L.');
      expect(result).toHaveProperty('cif', '39176747');
      expect(result).toHaveProperty('active', true);
      expect(result).toHaveProperty('anafData');
      expect(result.anafData.name).toBe('LSEG BUSINESS SERVICES RM S.R.L.');
    });
    ...
    it('should throw when ANAF returns no company name', async () => {
      mockFetch.mockResolvedValueOnce(anafCompanyResponse({ cui: 39176747, name: null }));
  ...
  describe('getCompanyData (with cache)', () => {
    ...
    it('should use cached company data when available', async () => {
      ...
      expect(result.cif).toBe('39176747');
  ...
  describe('validateAndGetCompany', () => {
    ...
    it('should return company data with status active', async () => {
      mockFetch
        .mockResolvedValueOnce(anafCompanyResponse(LSEG_ANAF_RECORD))
        .mockResolvedValueOnce(solrResponse(5, [
          { url: 'https://test.com/1', title: 'Job 1' },
          { url: 'https://test.com/2', title: 'Job 2' }
        ]))
        .mockResolvedValueOnce(peviitorResponse([{ company: 'LSEG BUSINESS SERVICES RM S.R.L.' }]));
