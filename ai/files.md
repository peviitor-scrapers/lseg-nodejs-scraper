...
| `tests/company.json` | Mock ANAF company data for LSEG used in unit tests |
| `tests/validate-lseg-jobs.js` | **LSEG-specific fast validator (used by CI).** HEAD requests only, hardcoded LSEG CIF. Called nightly by `automation-testing.yml`. Supports `--dry-run` and `--delete`. |
...
| `tests/e2e/scraper.test.js` | E2E tests - full pipeline with real LSEG career page, ANAF, and SOLR |
...
| `ROBOTS.md` | robots.txt analysis and scraping policy for LSEG Careers |
...
- Full workflow: validate company (ANAF+CUIScan+CUIFirma+Peviitor) → scrape LSEG → transform → upsert SOLR → generate docs/jobs.md
