...
```bash
gh run view <RUN_ID> --repo sebiboga/lseg-nodejs-scraper --json status -q .status
```
...
**On every session:**
1. Check open GitHub issues: `gh issue list --repo sebiboga/lseg-nodejs-scraper --state open`
