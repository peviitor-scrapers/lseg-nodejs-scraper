...
## 🌱 This Repo Follows the Peviitor Scraper Standard
This repo is a **standard peviitor scraper** for LSEG. The source of truth for company identity is `scraper/config/company.json`.
...
```bash
gh run view <RUN_ID> --repo peviitor-scrapers/lseg-nodejs-scraper --json status -q .status
```
...
**On every session:**
1. Check open GitHub issues: `gh issue list --repo peviitor-scrapers/lseg-nodejs-scraper --state open`
