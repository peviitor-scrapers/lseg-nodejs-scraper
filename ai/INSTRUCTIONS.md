...
This scraper extracts job listings from LSEG careers page (Romania only) and imports them to peviitor.ro.

Target: https://lseg.wd3.myworkdayjobs.com/en-US/Careers
...
1. **Start with brand** - We know the brand (e.g., "LSEG")
...
8. **Scrape new jobs** - Extract jobs from LSEG careers page (Romania)
...
> **Important**: Scraper does NOT delete jobs from other sources (ANOFM, etc). It only upserts LSEG Careers jobs. Existing jobs are preserved.
...
3. **Scrape jobs** - Extract jobs from LSEG careers API (Romania only)
...
company.js (validate company)
    ...
    │
    ▼ (if active)
scrape LSEG API (jobs for Romania)
    │
    ▼
